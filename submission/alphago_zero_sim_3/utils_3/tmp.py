...
class MCT(object):
    ...
    def actionProb(self, board, player, temp=1, greedy=False):
        """
        ...
        for _ in range(self.numSimulations):
            # try:
            self.search(board, player, greedy=greedy)
    ...
    def search(self, board, player, greedy):
        """
        ...
        if s not in self.Ps:
            ...
            if(not greedy):
                ...
                self.Ps[s], v = greedy_action(board, player)
            #------------------------------#
        ...
        v = self.search(next_s, next_player, greedy)
    ...
    def greedy_action(self, board, player):
        val= self.game.get_score(board, player)
        possible_actions = self.game.get_valid_moves(cur_board, player)
