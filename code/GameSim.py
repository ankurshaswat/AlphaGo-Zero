...
class GoState(object):
    ...
    def __init__(self, board, color):
        ...
        '''
        assert color in [pachi_py.BLACK,
                         pachi_py.WHITE], 'Invalid player color'
...
def make_pachi_policy(board, engine_type='uct', threads=1, pachi_timestr=''):
    engine = pachi_py.PyPachiEngine(
        board, engine_type, six.b('threads=%d' % threads))
...
class GoEnv(gym.Env):
    ...
    def __init__(self, player_color, observation_type, illegal_move_mode, board_size, komi):
        ...
        """
        self.saved_actions = []
        self.init_args = [player_color, observation_type,
                          illegal_move_mode, board_size, komi]
    ...
    def reset(self):
        self.state = GoState(pachi_py.CreateBoard(
            self.board_size), pachi_py.BLACK)
        print('J')
        print(self.state)
        print('J')
        ...
        if self.state.color != self.player_color:
            self.state, opponent_resigned, _ = self._exec_opponent_play(
    ...
    def decide_winner(self):
        ...
        reward = 1. if player_wins else - \
            1. if (white_wins or black_wins) else 0.
        return self.state.board.encode(), reward, True, {'state': self.state}, current_score

    def set_actions(self, action_arr):
        self.saved_actions = action_arr

    def step(self, action):
        new_actions = self.saved_actions[:]
        new_actions.append(action)

        new_env = GoEnv(self.init_args[0], self.init_args[1],
                        self.init_args[2], self.init_args[3], self.init_args[4])
        new_env.set_actions = new_actions
        for prev_action in new_actions:
            ret_val = new_env.step_actual(prev_action)

        return ret_val

    # Game terminates
    # return obs_t, r_t, done, info, cur_score
    def step_actual(self, action):
        ...
        try:
            ...
            self.last_player_passed = True if action == _pass_action(
                self.board_size) else False
            current_score = self.state.board.official_score + self.komi
    ...
    def _reset_opponent(self, board):
        if self.opponent == 'random':
            ...
            self.opponent_policy = make_pachi_policy(board=board, engine_type=six.b(
                'uct'), pachi_timestr=six.b('_2400'))  # TODO: strength as argument
        else:
            raise error.Error(
